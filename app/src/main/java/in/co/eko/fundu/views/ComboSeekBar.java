...
public class ComboSeekBar extends SeekBar {
	...
	private int mColor;
	private int mUnSlectedLineColor;
	...
	public ComboSeekBar(Context context, AttributeSet attrs) {
		...
		mUnSlectedLineColor = a.getColor(R.styleable.ComboSeekBar_unSelectedLine, Color.WHITE);
		...
		setThumb(mThumb);
		setProgressDrawable(new CustomDrawable(this.getProgressDrawable(), this, mThumb.getRadius(), mDots, mColor,mUnSlectedLineColor, mTextSize, mIsMultiline));
	 ...
	 *            color.
	 */
	public void setColor(int color, int colorInselectedLine) {
		mColor = color;
		mThumb.setColor(color);
		setProgressDrawable(new CustomDrawable(this.getProgressDrawable(), this, mThumb.getRadius(), mDots, color, colorInselectedLine, mTextSize, mIsMultiline));
	}
	...
	public synchronized void setSelection(int position) {
		...
		for (Dot dot : mDots) {
			if (dot.id == position) {
				dot.isSelected = true;
				dot.isSmallCircle = false;
				/*if (position%2==1) {
					dot.isSmallCircle = true;
				} else {
					dot.isSmallCircle = false;
            ...
            dot.isShouldSelected = dot.id < position;
		}
	...
	public void setAdapter(List<String> dots) {
		...
		for (String dotName : dots) {
			Dot dot = new Dot();
			int range = Integer.parseInt(dotName);
			/*if(range%1000==500){
				dot.text = "";
				dot.isSmallCircle = true;
			} else {*/
	...
	public void setIntTypeAdapter(List<Integer> dots) {
		mDots.clear();
		int index = 0;
		for (Integer dotName : dots) {
			Dot dot = new Dot();
			int range = dotName;
			if(range%1000==500){
				dot.text = "";
				...
				dot.text = String.valueOf(dotName);
			}
			dot.price = String.valueOf(dotName);
			dot.id = index++;
			mDots.add(dot);
		}
		initDotsCoordinates();
		...
		if ((mThumb != null) && (mDots.size() > 1)) {
			if (isSelected) {
				...
				if ((mDots.get(mDots.size() - 1).mX - bounds.centerX()) < 0) {
					...
					for (Dot dot : mDots) {
						dot.isSelected = false;
						//dot.isShouldSelected = false;
					}
					mDots.get(mDots.size() - 1).isSelected = true;
					mDots.get(mDots.size() - 1).isShouldSelected = true;
					handleClick(mDots.get(mDots.size() - 1));
					for(int j= 0; j<mDots.size();j++){
						mDots.get(j).isShouldSelected = true;
					}
				} else {
					...
					for (int i = 0; i < mDots.size(); i++) {
						if (Math.abs(mDots.get(i).mX - bounds.centerX()) <= (intervalWidth / 2)) {
							...
							mDots.get(i).isShouldSelected = true;
							selectedPosition = i;
							handleClick(mDots.get(i));
						} else {
							mDots.get(i).isSelected = false;
							mDots.get(i).isShouldSelected = false;
						}
	...
	public static class Dot {
		...
		public boolean isShouldSelected = false;
		public boolean isSmallCircle = false;
		public String price;
