...
public class TakeImage extends Activity {
	...
	private final String capture_dir = Environment
			.getExternalStorageDirectory() + "/TakeImageDemo/";
	Activity activity;
			...
			if (image_from.equalsIgnoreCase("camera")) {
				...
				if (takePictureIntent.resolveActivity(getPackageManager()) != null) {
					...
					try {
						...
						imageFileUri = FileProvider.getUriForFile(TakeImage.this,
								getString(R.string.file_provider_authority),
								file);

					} catch (Exception ex) {
						Fog.e("TakePicture", ex.getMessage());
					}
		...
		if (resultCode == RESULT_OK) {
			if (requestCode == REQ_CODE_PICK_IMAGE) {
				...
				if (selectedImage != null) {
					...
					cursor.close();
					Intent intent = new Intent();
					Fog.d("path","path"+path);
					 path = "file://"+path;
					intent.putExtra("filePath", path);
					AppPreferences.getInstance(this).putString(Constants.PROFILE_PIC_URL,path);
